...
local hl_char_two = blend(hl_char_one, "#565c64", 0.5)
...
local function custom_motion(kwargs)
  ...
  local function get_pattern(input, _)
  ...
  local function get_targets(pattern, _)
    local search = require "leap.search"
    local bounds = search["get-horizontal-bounds"]()
    local get_char_at = require("leap.util")["get-char-at"]
    local targets = {}
    for pos in search["get-match-positions"](pattern, bounds, { ["backward?"] = kwargs.cc.backward }) do
...
local create_mappings = function()
  ...
  local mappings = {
    ["<leader>j"] = { backward = false },
    ["<leader>J"] = { backward = true },
  }
